...
public class RobotExclusion {
     ...
     */
    public synchronized void add(String server, Vector perm) {
        ht.put(server, perm);
        ...
        if (ht.size() > maxSize) {
            ...
            //System.out.println(hostToRemove+ " with timestamp: "+ time +" removed at "+ System.currentTimeMillis());
        }
        if (ht.size() > maxSize || timeStamp.size() > maxSize || timeStamp.size() != ht.size()) {
            System.err.println("Fatal Error: Robot Exclusion cache exceeded");
            System.exit(1);
        }
    }
     ...
     */
    public synchronized Vector get(String server) {
    ...
    public static Vector getVector(String content) {
        ...
        Pattern p = Pattern.compile("\\s*User-agent\\s*:(.*)", Pattern.CASE_INSENSITIVE);
        Pattern pd = Pattern.compile("\\s*Disallow\\s*:(.*)", Pattern.CASE_INSENSITIVE);
        boolean add = false;
        for (int i = 0; i < lines.length; i++) {
            ...
            if (m.find()) {
                ...
                if (userAgent.trim().equals("*") || userAgent.trim().equals("Generic")) {
